...
### 8.7 批量自动化测试基础设施（`batch_test.py`）

为高效完成 V0-VN 各版本的性能对比实验，本组开发了批量测试脚本 `batch_test.py`，作为系统架构中的**自动化实验评估层**。该脚本解决三个痛点：

1. **手工测试效率低**：每次运行需手动启动 server → student → 打开前端点开始 → 等 300s 仿真结束。一轮 7 版本 × 3 种子 = 21 次手工操作不可行。
2. **结果难以追溯**：手动记录分数容易出错，缺少录像与分数的对应关系。
3. **参数一致性难保证**：不同版本间若用了不同的随机种子或速度参数，对比失去公平性。

#### 核心能力

| 功能 | 说明 |
| --- | --- |
| **进程生命周期管理** | 自动启动/停止 `game_server` 和 student 进程，测试结束自动清理端口 |
| **高速仿真** | 使用 `--speed 1000` 参数，300s 仿真约 6-9 秒跑完 |
| **多随机种子** | 支持指定种子列表或 `--runs N` 自动生成 N 个种子，消除单次运行的偶然性 |
| **Viewer 自动触发** | 脚本模拟前端 viewer 连接、发送 `start_game` 后断开，无需人工操作浏览器 |
| **指标自动提取** | 从录像文件的 `metadata` 字段读取完成订单数、订单收益、投递收益、碰撞扣分、超时扣分五项指标 |
| **录像归档** | 按 `{version}_seed{seed}.json` 命名保存，便于回放分析 |
| **汇总输出** | 终端打印汇总表格 + 保存 `batch_results.json`，各版本平均值一目了然 |
| **全部版本一键测试** | `python batch_test.py --all --runs 3` 自动测试 V1-V5+VN 各 3 个种子 |
...
# 全部版本一键测试
python batch_test.py --all                   # 各 3 个种子
python batch_test.py --all --runs 5          # 各 5 个种子
```

#### 输出示例

```text
Versions: ['student_v1', 'student_v2', 'student_v3', 'student_v4', 'student_v5', 'student_vn']
Seeds:    [1, 2, 3]
Speed:    1000x

[student_v1]
  seed=1 ... 2850.5pts   -> student_v1_seed1.json
  seed=2 ... 2910.2pts   -> student_v1_seed2.json
  seed=3 ... 2780.8pts   -> student_v1_seed3.json
...
---------------------------------------------------------------
student_v1     2847.2    24.3  2890.0   1040.0   1020.0    62.7     3
student_v2     2910.5    25.0  2950.0   1050.0    950.0    55.3     3
...
```
...
┌─────────────────────────────────────────────────────┐
│                  batch_test.py                      │
│  (实验评估层 — 自动化执行 + 指标采集 + 对比汇总)      │
├─────────────────────────────────────────────────────┤
│  进程管理  │  viewer模拟  │  指标提取  │  汇总输出    │
└──────┬──────┴──────┬──────┴──────┬─────┴──────┬─────┘
       │             │             │            │
       ▼             ▼             ▼            ▼
  game_server   student_v*.py  recordings/  batch_results.json
```

该脚本使实验流程从"手工操作 21 次"压缩为"一行命令全自动"，且所有结果可复现、可追溯，显著提升了实验效率和结论的可信度。
